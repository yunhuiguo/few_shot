...
def test_loop(novel_loader, return_std = False, loss_type="softmax", n_query = 15, n_way = 5, n_support = 5): #overwrite parrent function
    ...
    for _, (x, y) in enumerate(novel_loader):
        ...
        conv4_model = model_dict["Conv4"]()
        conv6_model = model_dict["Conv6"]()
        resnet10_model = model_dict["ResNet10"]()
        resnet18_model = model_dict["ResNet18"]()
        resnet34_model = model_dict["ResNet34"]()

        ###############################################################################################      
        checkpoint_dir = '%s/checkpoints/%s/%s_%s' %(configs.save_dir, "miniImageNet", "Conv4", "baseline")
        if params.train_aug:
            checkpoint_dir += '_aug'
        ...
        conv4_model.load_state_dict(state)
        ###############################################################################################      

        ###############################################################################################      
        checkpoint_dir = '%s/checkpoints/%s/%s_%s' %(configs.save_dir, "miniImageNet", "Conv6", "baseline")
        ...
        conv6_model.load_state_dict(state)

        ###############################################################################################
        checkpoint_dir = '%s/checkpoints/%s/%s_%s' %(configs.save_dir, "miniImageNet", "ResNet10", "baseline")
        ...
        resnet10_model.load_state_dict(state)

        ###############################################################################################
        checkpoint_dir = '%s/checkpoints/%s/%s_%s' %(configs.save_dir, "miniImageNet", "ResNet18", "baseline")
        ...
        resnet18_model.load_state_dict(state)
        ###############################################################################################
        checkpoint_dir = '%s/checkpoints/%s/%s_%s' %(configs.save_dir, "miniImageNet", "ResNet34", "baseline")
        ...
        resnet34_model.load_state_dict(state)
        ###############################################################################################
        ...
        conv4_embeddings = []
        for idx, module in enumerate(conv4_model.trunk):
            x_a_i = module(x_a_i)
            if len(list(x_a_i.size())) == 4:
                embedding =  F.adaptive_avg_pool2d(x_a_i, (1, 1)).squeeze()
                conv4_embeddings.append(embedding.detach())
      

        x_a_i = x_var[:,:n_support,:,:,:].contiguous().view( n_way* n_support, *x.size()[2:]) # (25, 3, 224, 224)
        conv6_embeddings = []
        for idx, module in enumerate(conv6_model.trunk):
            x_a_i = module(x_a_i)
            print (module)
            print (x_a_i.size())
            ...
            if len(list(x_a_i.size())) == 4:
                embedding =  F.adaptive_avg_pool2d(x_a_i, (1, 1)).squeeze()
                conv6_embeddings.append(embedding.detach())
        
        for embedding in conv6_embeddings:
            print (embedding.size())

        break


        conv6_embeddings = conv6_embeddings[4:-1]
        
        x_a_i = x_var[:,:n_support,:,:,:].contiguous().view( n_way* n_support, *x.size()[2:]) # (25, 3, 224, 224)
        resnet10_embeddings = []
        for idx, module in enumerate(resnet10_model.trunk):
            x_a_i = module(x_a_i)
            if len(list(x_a_i.size())) == 4:
                embedding =  F.adaptive_avg_pool2d(x_a_i, (1, 1)).squeeze()
                resnet10_embeddings.append(embedding.detach())
        resnet10_embeddings = resnet10_embeddings[4:-1]
    
        '''
        x_a_i = x_var[:,:n_support,:,:,:].contiguous().view( n_way* n_support, *x.size()[2:]) # (25, 3, 224, 224)
        resnet18_embeddings = []
        for idx, module in enumerate(resnet18_model.trunk):

            x_a_i = module(x_a_i)
            if len(list(x_a_i.size())) == 4:
                embedding =  F.adaptive_avg_pool2d(x_a_i, (1, 1)).squeeze()
                resnet18_embeddings.append(embedding.detach())
    

        resnet18_embeddings = resnet18_embeddings[4:-1]



        x_a_i = x_var[:,:n_support,:,:,:].contiguous().view( n_way* n_support, *x.size()[2:]) # (25, 3, 224, 224)
        resnet34_embeddings = []
        for idx, module in enumerate(resnet34_model.trunk):
            x_a_i = module(x_a_i)
            if len(list(x_a_i.size())) == 4:
                embedding =  F.adaptive_avg_pool2d(x_a_i, (1, 1)).squeeze()
                resnet34_embeddings.append(embedding.detach())
        resnet34_embeddings = resnet34_embeddings[4:-1]
        '''
        ##########################################################
