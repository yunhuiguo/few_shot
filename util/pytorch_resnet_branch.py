...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=[]):
        ...
        self.branches = []
        self.branches.append(self._make_layer(block, 512, layers[3], stride=2))
        for _ in range(len(self.num_classes)-1):
            self.branches.append(copy.deepcopy(self.branches[0]))
        ...
        for i in range(len(self.num_classes)):
            self.fcs.append(nn.Linear(512 * block.expansion, self.num_classes[i]))
